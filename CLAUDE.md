...
- **Live URL:** **https://stonesriveralbum.com** (custom domain; verified 2026-09-11 serving the same bundle as Cloud Run). Underlying Cloud Run service `stones-river-album` (us-east1): https://stones-river-album-bh327oxava-ue.a.run.app (or `gcloud run services describe stones-river-album --region=us-east1 --project=yonas-media-agent --format='value(status.url)'`)
...
## Versioning
- `VERSION` file in repo root, currently **`2.17.10`**.
...
## Current State (as of 2026-09-11, v2.17.10)
- **The album title *Stones River* is italicized everywhere it appears in copy (v2.17.8, OPO style note).** `italicizeAlbumTitle()` in `src/app/utils/albumTitle.tsx` wraps each "Stones River" in `<em>`. It's applied in About, Full Bio, Quote, the preorder cards/modals/item names, and the cart drawer, and `italicizeAlbumTitleHtml()` does the same to Shopify tier descriptions at the fetch boundary. "Battle of Stones River" (Full Bio) is the 1862 battle, not the album, so the regex skips it. **Route any new copy that names the album through the helper.** Alt text and the page `<title>` can't carry italics and are left alone. Headings are included (About `h2`, "Preorder *Stones River*"); the Garamond heading face has no italic file, so those render as a browser-synthesized slant, while Fira Sans body text has a real italic. That fake slant leans the last letter into the following space ("River weaves" looked jammed, Ben caught it), so `global.css` gives `.font-garamond em` a `padding-right: 0.1em` italic correction (v2.17.10). If the real ITC Garamond Condensed Light Italic file ever arrives (designer Ben Tousley may have it), register it as an `@font-face` with `font-style: italic` and drop the padding.
- **Video poster is now a frame from the Revision 3 cut (v2.17.9).** The old designed thumbnail showed two tubas lying on the riser, bottom right, and Jeremy wanted them gone. The new video's opening is framed without them, so the poster is now its title card at 7.5s (regen command in the Video bullet below).
...
## In Flight / Blocked on Ben (as of 2026-09-11)
- **Shopify tier descriptions still carry placeholder dates, visible on the live site.** Tiers 1 and 2 say "Orders placed by XX will ship on XX" and Tier 3 lists the listening session as "Date TBD". The tier modal renders Shopify's `descriptionHtml`, so fans see the XX. Needs real ship-by dates from Ben; then either someone edits them in Shopify admin or the site patches them at the fetch boundary (same spot as `fixDoubleEscapedEntities`).
- **OPO feedback list (YM task, owner Lauren): every site-side item is done as of v2.17.10.** That covers name order, Eric's title, Jacobsen spelling, Brooklyn Rider removed, no "the" before OPO, photo credits, Tier 5 ampersand (site-side), the Associate Producer section, and *Stones River* italics. Only the Shopify ship dates above remain; after that, the task's last step is sending OPO the site for final sign-off.
...
## Dead Ends — Do Not Retry (2026-08-19, 2026-09-11)
- **Storefront `products(first: 20)` returns Jeremy's old catalog, not the preorder tiers.** Query the six tier products by the gids in `preorderData.ts` via `nodes(ids:)` instead.
- **`gcloud builds list --filter="substitutions.SHORT_SHA=..."` can take 2+ minutes** (it scans the whole project's history). The awk-over-`--limit=8` recipe below is instant.
- **Grepping the live JS bundle for `Teaser Thumbnail-<hash>` finds nothing** because Vite URL-encodes the space. Grep `Teaser%20Thumbnail`, or md5 the served file against the repo copy (that's what proved the v2.17.9 poster swap).
- **Headless Chrome (`--headless=new --screenshot`) writes the PNG but never exits** on this Mac. Run it in the background with its own `--user-data-dir`, then `pkill -f` that dir. It's the way to render type previews from a local HTML file (embed the OTF as a base64 data URI) without running a server.
- **There is no 4K master of the Revision 3 sizzle on this Mac** (searched `~` five levels deep, 2026-09-11), so the v2.17.9 poster was grabbed from the 1080p web encode. If a sharper poster is ever wanted, ask Ben for the master.
...
- **Video:** sizzle reel is **Revision 3** (swapped 2026-09-08, v2.17.7), re-encoded to ~29MB to stay under Cloud Run's ~32MiB response cap. If you swap the video, keep it under that ceiling or it 500s. **Working recipe** for the 4K masters Ben drops on his Desktop (~1GB, 3840x2160 @ 58Mbps): two-pass x264 `-vf scale=1920:1080:flags=lanczos -preset slow -b:v 1500k -c:a aac -b:a 128k -movflags +faststart` -> ~28.9MB for a 141s cut. The file path `public/assets/stones-river-sizzle.mp4` is referenced in exactly one place (`VideoSection.tsx`), so a swap is a drop-in file replace with no code change. The `poster` (`assets/Teaser Thumbnail.png`) is a frame grab of the title card at 7.5s (v2.17.9), so **if the video is swapped again, regenerate the poster too**: `ffmpeg -ss 7.5 -i public/assets/stones-river-sizzle.mp4 -frames:v 1 "assets/Teaser Thumbnail.png"`. Pick a moment where the title has fully faded in but the ~8s dissolve hasn't started. `VideoSection.tsx` imports it by that filename, so keep the name and it's a drop-in replace. The preview is sized `max-w-6xl` with trimmed gutters (`VideoSection.tsx`) so it fills more of the blue texture band while leaving top/bottom margin.
